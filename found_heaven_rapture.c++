...
// Custom EtherType for bare-metal sender
...
void packetHandler(u_char* userData, const struct pcap_pkthdr* pkthdr, const u_char* packet) {
    if (pkthdr->len < sizeof(EthernetHeader) + 5) return; // must contain header + seq + flags
...
int main(int argc, char* argv[]) {
    ...
    std::cout << "Listening on " << dev << " for EtherType 0x" << std::hex << CUSTOM_ETHER_TYPE << std::dec << "...\n";
